...
$sql="SELECT tblbooking.UserID, tbluser.FullName,tbluser.MobileNumber,tbluser.Email,tblbooking.BookingID,tblbooking.BookingDate,tblbooking.BookingFrom,tblbooking.BookingTo,tblbooking.EventType,tblbooking.Numberofguest,tblbooking.Message, tblbooking.Remark,tblbooking.Status,tblbooking.UpdationDate,tblservice.ServiceName,tblservice.SerDes,tblservice.ServicePrice from tblbooking join tblservice on tblbooking.ServiceID=tblservice.ID join tbluser on tbluser.ID=tblbooking.UserID  where tblbooking.ID=:eid";
$query = $dbh -> prepare($sql);
$query-> bindParam(':eid', $eid, PDO::PARAM_STR);
$query->execute();
...
<?php
   if(isset($_POST['submit'])){
    $s=$_POST['status'];
    $uid=$row->UserID;
    $sql ="INSERT INTO `notifications` (`nid`, `uid`,`read`, `status`, `date`) VALUES (NULL, '$uid','0', '$s', CURRENT_TIMESTAMP)";
    $query=$dbh->prepare($sql);
    $query->execute();
    if($s=="Approved"){
    $to_email = " echo $row->Email";
    $subject = "Booking Confirmed";
    $body = "Yay,
          Your booking have been confirmed. We will contact you soon for further processing. Let's make your event the best one.
          Feel free to mail us for more details.
          Thank you.
          Best regards.";
    $headers = "From: ritaadhikari796@gmail.com";
    }
    else{
      $to_email = "echo $row->Email";
      $subject = "Booking Cancelled";
      $body = "Sorry Your Booking have been cancelled because of unavailability of Banquet for the requested date. Hope you will find the best place.
      Please, Remember us for the events next time.
      Thank you.
      Best regards.";
      $headers = "From: ritaadhikari796@gmail.com";
